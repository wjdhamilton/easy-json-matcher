...
class ValidationStepTest < ActiveSupport::TestCase
  ...
  setup do
    @subject = EasyJSONMatcher::ValidationStep.new(verify_with: ->(value, errors){})
  end 
  ...
  test "It should send call to the verifier assigned to it" do
    ...
    subject = EasyJSONMatcher::ValidationStep.new(verify_with: verifier)
  ...
  test "If the verifier returns :stop the chain will be halted" do
    ...
    middle = EasyJSONMatcher::ValidationStep.new(verify_with: verifier)
    tail = MiniTest::Mock.new
    test_value = "verify me"
    @subject >> middle >> tail
    @subject.check(value: test_value)
    # There is no official way to check that something wasn't called in MiniTest, so we'll
    # just have to assume that since verify passes, then check wasn't called in tail
    tail.verify
  end

  test "It should return an array of errors found" do
    head = EasyJSONMatcher::ValidationStep.new(verify_with: ->(value, errors) { errors << 1 })
    tail = EasyJSONMatcher::ValidationStep.new(verify_with: ->(value, errors) { errors << 2 })
    head >> tail
